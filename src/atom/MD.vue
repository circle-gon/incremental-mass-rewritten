<template>
  ...
  <h4>
    ...
    {{ formatGain(player.md.mass, MASS_DILATION.dilatedMassGain.value, true) }}
  </h4>
  of dilated mass, which gives a
  <h4>{{ formatMult(MASS_DILATION.dilatedMassEffect.value) }}</h4>
  to Tickspeed power.<br />
  Relativistic Particle gain formula - (log<sub>10</sub>([mass]) / 50 - 9)<sup
    >{{ format(MASS_DILATION.rpExp.value) }}</sup
  >
  * {{ format(MASS_DILATION.rpMult.value) }}<br /><br />
  <Tooltip>
    <template #content>
      <button class="btn full md" @click="MASS_DILATION.run()">
        {{ dilateText }}
      </button>
    </template>
    <template #tooltip>
      {{ MASS_DILATION.rpText.value }}
    </template>
  </Tooltip>
  <br /><br />
    ...
    >
      <div style="min-height: 80px">
        <template v-if="upg.max > 1">
          [Level {{ formatInteger(upg.cost.amt.value)
          }}{{
            upg.max < Infinity ? " / " + formatInteger(upg.max) : ""
          }}]<br />
        </template>
        {{ unref(upg.desc) }}.<br />
      ...
      {{
        upg.cost.amt.value.lt(upg.max)
          ? "Cost: " + formatMass(upg.cost.cost.value)
          : ""
      }}
    </button>
  </div>
...
const dilateText = computed(() => {
  ...
  if (gain.gte(1))
    return `Cancel dilation for ${formatInteger(gain)} Relativistic Particles`;
