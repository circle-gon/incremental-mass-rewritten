<template>
  <div id="container">
    ...
    <canvas id="graph" ref="graph" width="800" height="600"></canvas>
    <div>
      <Tooltip>
        <template #content>
          <button
            class="btn"
            :class="done ? 'locked' : null"
            @click="offlineProgress.speed++"
          >
            Speed Up
          </button>
        </template>
        <template #tooltip> Offline Progress will run twice as fast. </template>
      </Tooltip>
      <Tooltip>
        <template #content>
          <button
            class="btn"
            :class="done ? 'locked' : null"
            @click="offlineProgress.speed = Infinity"
          >
            Finish
          </button>
        </template>
      ...
      <button class="btn" :class="!done ? 'locked' : null" @click="end">
        Done
      </button>
    </div>
  </div>
